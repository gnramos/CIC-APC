...
class Data():
    def __init__(self):
        # Função de inicialização de valores, é executada
        # sempre que uma nova instância é criada.
        dia, mes, ano = 0, 0, 0  # A "data" em si.
        descricao = ''           # Uma descrição da data.

data = Data()
# Como um registro não é um tipo primitivo, é preciso
# declarar a variável como tal. A instrução "Data()" cria
# uma nova instância.

data.descricao = input('Digite a descrição: ')
# Armazene a informação lida da entrada padrão no espaço
# identificado como "descricao" dentro da variável "data"
# (registro).

data.ano = int(input('Digite o ano: '))
# Armazene a informação lida da entrada padrão no espaço
# identificado como "ano" dentro da variável "data"
# (registro).

data.mes = int(input('Digite o mês: '))
# Armazene a informação lida da entrada padrão no espaço
# identificado como "mes" dentro da variável "data"
# (registro).

data.dia = int(input('Digite o dia: '))
# Armazene a informação lida da entrada padrão no espaço
# identificado como "da" dentro da variável "data"
# (registro).
